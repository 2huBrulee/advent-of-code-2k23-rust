...
impl Hand {
    fn get_type(&self) -> HandType {
        ...
        let mut joker_count = *card_map.get("J").unwrap_or(&0);

        if joker_count < 5 && joker_count >= 1 {
            card_map.remove("J");

            let mut highest_card = "";
            let mut highest_card_count = 0;

            for (card, count) in &card_map {
                if *count > highest_card_count {
                    highest_card = card.as_str();
                    highest_card_count = *count;
                }
            }

            card_map.insert(highest_card.to_string(), highest_card_count + joker_count);
        }
